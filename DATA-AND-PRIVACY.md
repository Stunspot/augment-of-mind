...
The `1.0.0` plugin is a bundle of local instruction, reference, schema,
template, and evaluation files. It contains no MCP server, telemetry client,
analytics script, tracker, credential, or hosted data service.

Installing the plugin makes its skills available to the host. What the host or
model receives in a task still depends on the host, enabled tools, user input,
workspace policy, and provider terms. The plugin does not bypass those controls.
