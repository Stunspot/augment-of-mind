...
MIGRATOR_PATH = (
    ROOT / "skills" / "augment-of-mind" / "scripts" / "migrate_mission_capsule.py"
)
LEGACY_FIXTURE = ROOT / "tests" / "fixtures" / "mission-capsule-v1.json"
MIND_SKILL = ROOT / "skills" / "augment-of-mind" / "SKILL.md"
RUNTIME = (
    ROOT
    / "skills"
    / "augment-of-mind"
...
EVAL_DIR = ROOT / "skills" / "augment-of-mind" / "evals"
EVAL_MANIFEST = EVAL_DIR / "eval-manifest.yaml"
EVAL_CASES = EVAL_DIR / "faculty-runtime-cases.yaml"
...
class MindMissionCapsuleTests(unittest.TestCase):
    @classmethod
    ...
    def test_behavioral_eval_covers_resolution_fidelity(self) -> None:
        manifest = json.loads(EVAL_MANIFEST.read_text(encoding="utf-8"))
        suite = json.loads(EVAL_CASES.read_text(encoding="utf-8"))
        self.assertEqual(manifest["package_version"], suite["package_version"])
        self.assertIn(
            "pursuit_resolution_fidelity", manifest["indispensable_dimensions"]
        )
        self.assertIn(
            "capsule_version_recoverability", manifest["indispensable_dimensions"]
        )
        self.assertEqual(14, len(suite["cases"]))

        case = next(case for case in suite["cases"] if case["id"] == "HAFR-013")
        self.assertIn("pursuit_resolution_fidelity", case["dimensions"])
        expected = " ".join(case["expected_behaviors"])
        failures = " ".join(case["failure_signals"])
        self.assertIn("current_direction", expected)
        self.assertIn("omits acceptance entirely", expected)
        self.assertIn("without calling the pursuit complete", expected)
        self.assertIn("complete naturally", expected)
        self.assertIn("completion gate", failures)

        compatibility = next(
            case for case in suite["cases"] if case["id"] == "HAFR-014"
        )
        self.assertIn("capsule_version_recoverability", compatibility["dimensions"])
        expected = " ".join(compatibility["expected_behaviors"])
        failures = " ".join(compatibility["failure_signals"])
        self.assertIn("retained versioned v1 schema", expected)
        self.assertIn("legacy_v1", expected)
        self.assertIn("unresolved provenance", expected)
        self.assertIn("overwrites the source", failures)
